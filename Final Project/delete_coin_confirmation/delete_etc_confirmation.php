...
$old_coin_amt = "SELECT users_info.ec_amt FROM users_info WHERE users_info.username = '".$_SESSION['username']."';";
echo "<hr>";
...
echo $row_oldcoin["ec_amt"];

if ($row_oldcoin["ec_amt"]>0)
{
	echo "<hr>";

	$new_coin_amt = $row_oldcoin["ec_amt"]-1;
// $new_coin_amt=$old_coin_amt+1;
	echo "<hr>";
	echo $new_coin_amt;
	echo "<hr>";

	$new_portamt = $row_oldport["amt_holding"]-$etc_coin_price;
	echo $new_portamt;
	...
	$sql = "UPDATE users_info SET users_info.ec_amt = '" . $new_coin_amt . "', amt_holding = '" . $new_portamt . "' WHERE users_info.username = '".$_SESSION['username']."';";
	$mysqli->query($sql);
...
{
	$mysqli->close();
	header("Location: ../port.php?erroretc=You+can+not+have+less+than+zero+shares");
}

...
$old_coin_amt = "SELECT users_info.dc_amt FROM users_info WHERE users_info.username = '".$_SESSION['username']."';";
echo "<hr>";
...
echo $row_oldcoin["dc_amt"];

if ($row_oldcoin["dc_amt"]>0)
{

	echo "<hr>";

	$new_coin_amt = $row_oldcoin["dc_amt"]-1;

	echo "<hr>";
	echo $new_coin_amt;
	echo "<hr>";

	$new_portamt = $row_oldport["amt_holding"]-$dash_coin_price;
	echo $new_portamt;
	...
	$sql = "UPDATE users_info SET users_info.dc_amt = '" . $new_coin_amt . "', amt_holding = '" . $new_portamt . "' WHERE users_info.username = '".$_SESSION['username']."';";
	$mysqli->query($sql);
...
else
{
	...
	header("Location: ../port.php?errordash=You+can+not+have+less+than+zero+shares");

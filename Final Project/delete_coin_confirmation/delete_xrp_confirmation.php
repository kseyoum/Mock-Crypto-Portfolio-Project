...
$old_coin_amt = "SELECT users_info.rc_amt FROM users_info WHERE users_info.username = '".$_SESSION['username']."';";
echo "<hr>";
...
echo $row_oldcoin["rc_amt"];


if ($row_oldcoin["rc_amt"]>0)
{

	echo "<hr>";

	$new_coin_amt = $row_oldcoin["rc_amt"]-1;
// $new_coin_amt=$old_coin_amt+1;
	echo "<hr>";
	echo $new_coin_amt;
	echo "<hr>";

	$new_portamt = $row_oldport["amt_holding"]-$xrp_coin_price;
	echo $new_portamt;
	...
	$sql = "UPDATE users_info SET users_info.rc_amt = '" . $new_coin_amt . "', amt_holding = '" . $new_portamt . "' WHERE users_info.username = '".$_SESSION['username']."';";
	$mysqli->query($sql);
...
{
	$mysqli->close();
	header("Location: ../port.php?errorxrp=You+can+not+have+less+than+zero+shares");
}
